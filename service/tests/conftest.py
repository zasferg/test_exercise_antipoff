...
@contextmanager
def add_query_to_base(
    postgre_fixture: Engine, data: List[DataModel]
) -> Generator[List[DataModel], None, None]:
...
def add_history_to_base(
    ...
    with postgre_fixture.begin() as conn:
        for item in data:
            values = item.model_dump()
            sql_insert = insert(metadata.tables["service_queryhistory"]).values(
                **values
            )
            conn.execute(sql_insert)
    yield data
...
@pytest.fixture()
def add_query(
    postgre_fixture: FixtureRequest,
) -> Generator[List[DataModel], None, None]:

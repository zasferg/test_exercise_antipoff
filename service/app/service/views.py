...
class QueryPoint(APIView):
    ...
    def post(self, request: Request) -> Response:
        if not request.data:
            return Response(
                {"message": "Нет данных в запросе"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def get(self, request: Request) -> Response:
        try:
            response = requests.get(
                url=f"{EXTERNAL_SERVER_URL}/server/", timeout=60
            ).status_code
            if response < 500:
...
class ResultPoint(APIView):
    ...
    def get_object(self, cad_number: str) -> Union[Response, QueryHistory]:
        try:
            ...
            return Response(
                {"message": "Данной записи не существует"},
...
class HistoryPoint(APIView):
    ...
    def get(self, request, cad_num, format=None):
        if not cad_num:
            return Response(
                {"error": "Пустой кадастровый номер"},
        ...
        if not queries.exists():
            return Response(
                {"error": "Для этого кадастрового нмера нет истории запросов"},
                status=status.HTTP_404_NOT_FOUND,
            )
